...
{
	uint8_t x;
	RomData_ReadBytes(EEDataOriginAddr+0x9e,RomDatas, 2);
	x = RomDatas[0];
	x += RomDatas[1];
	...
	if (GetBit_uint8(RomDatas[0], 4) != 0)//学习过
	{
...
void UsedDEC(void)
{
	...
	RomData_WriteByte(EEDataOriginAddr+LeftTimes69Addr, 0xff);//原地址写0xff，写一个写入减一的
	if (LeftTimes69Addr == 0x69)
	{
		RomData_WriteByte(EEDataOriginAddr+0x70, LeftTimes69);
	}
	else
	{
		RomData_WriteByte(EEDataOriginAddr+LeftTimes69Addr - 1, LeftTimes69);
	}
}
//0xa0 0xa1 射频学习状态存储
//0xd0~0xee 存储当前hash值
...
void GetKeyParam()
{
	...
	if(!RomStateFlags.Bits.bRomWrited)
		return;
	//写过了，获取系统参数
	EE00 = RomData_ReadByte(EEDataOriginAddr+0x00);
	EE00 >>= 6;
	RomData_ReadBytes(EEDataOriginAddr+0x01, PSW, 8);
	...
	else
	{
		...
		if(!bFuncRet_IsSet() || t1!=LeftTimes69)//当前存储器中没有当前哈希值，则生成
		{
			...
			RomData_WriteBytes(EEDataOriginAddr+addr, RomDatas, 10);//写入当前次数段，且使其匹配
			for (i = 0; i < 8; i++)
			{
				curHash[i] = RomDatas[7 - i];
			}
...
void CheckDataBlockVerify(uint8_t Addr)
{
	uint8_t x;
	bFuncRet_Reset();
	RomData_ReadBytes(EEDataOriginAddr+Addr,RomDatas,10);
	if(RomDatas[8]>0x7f)
	{
		bFuncRet_Reset();
		return;
...
{
	if (RomData_ReadByte(EEDataOriginAddr+addr) != 0xff)
	{
		RomData_WriteByte(EEDataOriginAddr+addr, 0xff);
	}
	else
	{
		RomData_WriteByte(EEDataOriginAddr+addr, 0);
	}
}
...
{
	uint8_t i, x;
	RomData_ReadBytes(EEDataOriginAddr+0x69, RomDatas, 8);
	for (i = 8; i != 0; i--)
	{
		x = i - 1;
		if (RomDatas[x] != 0xff)
	...
	{
		LeftTimes69 = 0;
		RomData_WriteByte(EEDataOriginAddr+0x69, 0);
		// 		CalcTimes_D2=0;
		// 		CalcTimes_BF=0;
...
void VerifyEEDatas(uint8_t maxNum,uint8_t lastAddr)
{
	...
	for(i=0;i<maxNum;i++)
	{
		RomData_ReadBytes(EEDataOriginAddr+addr,RomDatas,8);
		if(addr==0xfe)
		{
			RomDatas[0]=0;
			RomDatas[1]=0;
	...
	RomData_ReadBytes(EEDataOriginAddr+lastAddr,RomDatas,8);
	for(i=0;i<8;i++)
	{
		if(ee[i]!=RomDatas[i])
		return;
...
void GetLeftTimeBlock(uint8_t nBlock)
{
	...
	{
		t1 = RomData_ReadByte(EEDataOriginAddr+addr1+8);
		t2 = RomData_ReadByte(EEDataOriginAddr+addr2 + 8);
		if (t1 < t2)
		{
			LeftTimesAddr[nBlock] = addr1;
			OtherLeftTimesAddr[nBlock] = addr2;
		...
		case LeftTimesH:
			...
			{
				RomData_ReadBytes(EEDataOriginAddr+0x0d, RomDatas, 8);
				RomDatas[8] = 0;
				RomDatas[9]=GetVerify_byteXOR(RomDatas,9);
				//RomDatas[9] = Verify.GetVerify_byteXOR(RomDatas, 9);
				RomData_WriteBytes(EEDataOriginAddr+LeftTimesAddr[LeftTimesH], RomDatas, 10);//写入当前次数段，且使其匹配
			}
			else
			{
				t2 = LeftTimes[LeftTimesH] - 1;
		...
		case LeftTimesM:
			if (LeftTimes[LeftTimesM] == 0)//中位为0，则需要向高位借位
			{
				RomData_ReadBytes(EEDataOriginAddr+LeftTimesAddr[LeftTimesH], RomDatas, 8);
				RomDatas[8] = 0;
				RomDatas[9]=GetVerify_byteXOR(RomDatas,9);
				//RomDatas[9] = Verify.GetVerify_byteXOR(RomDatas, 9);
				RomData_WriteBytes(EEDataOriginAddr+LeftTimesAddr[LeftTimesM], RomDatas, 10);
		...
		case LeftTimesL:
			...
			if (LeftTimes[LeftTimesL] == 0)//低位为0，则需要向中位借位
			{
				RomData_ReadBytes(EEDataOriginAddr+LeftTimes[LeftTimesM], RomDatas, 8);
				RomDatas[8] = 0;
				RomDatas[9]=GetVerify_byteXOR(RomDatas,9);
				//RomDatas[9] = Verify.GetVerify_byteXOR(RomDatas, 9);
				RomData_WriteBytes(EEDataOriginAddr+LeftTimesAddr[LeftTimesL], RomDatas, 10);
				if ((LeftTimes[LeftTimesM] == 0) && (LeftTimes[LeftTimesH] == 0))//中位、高位都为0，则无法借位，不再生成低位第二段
				break;
				else//借位
				{
