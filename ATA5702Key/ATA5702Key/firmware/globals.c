...
void ATA_globalsClkSwitchSrc_C(void)
{
	uint8_t bSreg = SREG;
...
void ATA_globalsClkStartXTO_C(void)
{
	...
	FETN4 = RomData_ReadByte(0x0904);//eepFETN4, 1U);
	ATA_SETBITMASK_C(FEEN1,_BM(XTOEN))
	FEBT = RomData_ReadByte(0x0903);//eepFEBT, 1U);
...
void ATA_globalsClkSwitchXTO_C(uint8_t bXtoClockSelect)
{
	/* LLR-Ref: 010 */
	uint8_t bSreg = SREG;
	uint8_t bCmcr = CMCR & 0x30U;
...
void ATA_calibInit_C(void)
{
	...
	RomData_ReadBytes(0x086a,ary,2U);
	...
	RomData_ReadBytes(wEepCalibAddr,pDestData,  sizeof(g_sCalibConfig.sSrcCalibrationConfig));
	...
	RomData_ReadBytes(wEepCalibAddr, pDestData, sizeof(g_sCalibConfig.sFrcCalibrationConfig));
}

void ATA_globalsCopySramSpace_C(uint8_t *pDestination, uint8_t *pSource, uint8_t bLength)
{
	/* LLR-Ref: 010 */
	if (bLength) {
		/* LLR-Ref: 020 */
