...
;-------
adr_12C8:
	CALL	sub_1381		;RAM_C4,RAM_C3,RAM_C2,获取
adr_12CB:
	CALL	sub_1330		;读取序号，并处理
...
sub_1E83:
	...
	CALL	sub_1ED3		;从EE的01地址开始读取8个数据到:RAM_8B,RAM_8C...RAM_92
...
adr_13A2:
	...
	CALL	ON_VL			;掉电中断
	CALL	sub_1442		;0x69起始的数据段:最低计数器单元进行减1操作
...
;
	...
	CALL	sub_15BE		;
adr_1587:
	CALL	sub_1663		;IR send:33 8F 90 61 CB
	MOVLW	0x2D			;
	BTFSS	RAM_C4,0,BANKED		;
	MOVLW	0x37			;
	BRA	adr_159D		;
...
adr_159D:
	...
	MOVF	PRODL,W,A		;
	MOVWF	RAM_BF,BANKED		;
	CALL	sub_14F0		;计算次数是当前值乘以0X80
adr_15B0:
	MOVLW	0x41			;
	BTFSC	RAM_C3,0,BANKED		;
	MOVLW	0x4B			;
	CALL	sub_1B7E		;计算出+9单元数据重写使数据段匹配
...
sub_162E:
	MOVWF	RAM_D1,BANKED		;
	CALL	sub_1BB6_XOR_xptoxp8_XOR_I2C0809_reW		;
	BNZ	adr_1642		;
...
sub_1BAF_END:
	RETURN				;
;---------------------------------------
sub_1BB6_XOR_xptoxp8_XOR_I2C0809_reW:
	CALL	sub_1B48_Load8EEto93to9A_byX_P		;
