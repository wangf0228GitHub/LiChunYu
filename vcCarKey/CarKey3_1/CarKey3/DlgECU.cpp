...
void ECU_DPassResultProc(void* p,byte* pBuf)
{
	CDlgECU* dlg=(CDlgECU*)(p);
	CString strTemp;
	if(pBuf!=NULL)
	{
		for(int i=0;i<8;i++)
...
IMPLEMENT_DYNAMIC(CDlgECU, CDialogEx)

CDlgECU::CDlgECU(CWnd* pParent /*=NULL*/)
	: CDialogEx(CDlgECU::IDD, pParent)
...
void CDlgECU::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgECU, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON1, &CDlgECU::OnBnReadInfo)
	ON_BN_CLICKED(IDC_BUTTON2, &CDlgECU::OnBnReadData)
...
//	ON_BN_CLICKED(IDC_BUTTON3, &CDlgECU::OnBnECUGetPass)
	ON_BN_CLICKED(IDC_BUTTON4, &CDlgECU::OnBnRenew)
	ON_EN_CHANGE(IDC_EDIT19, &CDlgECU::OnEnDealerEdit)
END_MESSAGE_MAP()
...
// 	RepositionBars(AFX_IDW_CONTROLBAR_FIRST,AFX_IDW_CONTROLBAR_LAST, 0);	
	...
	SetDlgItemText(IDC_BUTTON3,strTemp+_T(" ECU"));

	strTemp.LoadString(IDS_RenewPassCalc);
	SetDlgItemText(IDC_BUTTON4,strTemp); 

	strTemp.LoadString(IDS_ModleType);
	SetDlgItemText(IDC_STATIC25,_T("ECU")+strTemp); 
...
void CDlgECU::ShowLog( CString str )
{
	m_StatusBar.SetText(str, 0, 0);
}

void CDlgECU::OnBnReadData()
{
	//OnBnReadInfo();
	CString strTemp,strTemp1,strInfo;
	CInteractionData rx;
	...
	ShowLog(strTemp);
	CInteractionData Result;
	byte* pBuf=CCarKey3App::TheHIDDevice.ECUReadData(&m_Progress,&Result,&fEcuMd);
	if(pBuf==NULL)
	{
		ShowLog(Result.GetHIDResult());
		return;
...
// 	fileDlgS.m_ofn.lpstrTitle=strTemp;	
// 	fileDlgS.m_ofn.nMaxFile = MAX_PATH;	
// 	CString strFileName=_T("ECUData");
// 	fileDlgS.m_ofn.lpstrFile=strFileName.GetBuffer(MAX_PATH);
// 	if (IDOK==fileDlgS.DoModal())
// 	{
// 		CFile file(fileDlgS.GetPathName(),CFile::modeCreate|CFile::modeWrite);
...
void CDlgECU::OnEnDealerEdit()
{
	...
	{		
		...
		else
		{
			...
			for(int i=0;i<8;i++)
			{
				if(cpc.HashOut[i]!=ECUData[0x100+i])
				{
					bRight=false;
					break;
				}
...
void CDlgECU::EnableControl( BOOL bEn )
{
...
void CDlgECU::OnBnECUGetPass()
{

}

void CDlgECU::OnBnRenew()
{
	...
	{		
		...
		ShowLog(strTemp);
		CInteractionData Result;
		CCarKey3App::TheHIDDevice.ECURenew(DPSW,&m_Progress,&Result,fEcuMd);
	...
	else//获得密码
	{
		...
		else
		{
			...
			tParam->pDealer=&(ECUData[0x80]);
			tParam->pDPassResultProc=ECU_DPassResultProc;
...
void CDlgECU::ShowData()
{
	m_Key0.SetContent(CCommFunc::byteToHexStr(ECUData, 0, 8, _T(" ")));
	m_Key1.SetContent(CCommFunc::byteToHexStr(ECUData, 0x10, 8, _T(" ")));
	m_Key2.SetContent(CCommFunc::byteToHexStr(ECUData, 0x20, 8, _T(" ")));
	m_Key3.SetContent(CCommFunc::byteToHexStr(ECUData, 0x30, 8, _T(" ")));
	m_Key4.SetContent(CCommFunc::byteToHexStr(ECUData, 0x40, 8, _T(" ")));
	m_Key5.SetContent(CCommFunc::byteToHexStr(ECUData, 0x50, 8, _T(" ")));
	m_Key6.SetContent(CCommFunc::byteToHexStr(ECUData, 0x60, 8, _T(" ")));
	m_Key7.SetContent(CCommFunc::byteToHexStr(ECUData, 0x70, 8, _T(" ")));

	bDealerEdit=true;
	m_Dealer.SetContent(CCommFunc::byteToHexStr(ECUData, 0x80, 8, _T(" ")));
	bDealerEdit=false;
	m_SerialNum.SetContent(CCommFunc::byteToHexStr(ECUData, 0xa0, 8, _T(" ")));
	m_Transport.SetContent(CCommFunc::byteToHexStr(ECUData, 0x90, 8, _T(" ")));

	byte status=ECUData[0xb1]&0x0f;
