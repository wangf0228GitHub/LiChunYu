...
namespace 服务器端
{
    public class KeyDataWork
    {
        ...
        public static string workPath = "d:\\mb2\\";
        ...
        {
            FileInfo f;
            f = new FileInfo(workPath+"st12.bin");
            Stream stream = File.OpenRead(f.FullName);
            byte[] bin = new byte[stream.Length];
            stream.Read(bin, 0, (int)stream.Length);
            stream.Close();
            ...
            FileInfo f1;
            f1 = new FileInfo(workPath+"Calc.bin");
            ...
            FileInfo f2;
            f2 = new FileInfo(workPath+"Calc111.bin");
            Stream stream2 = File.OpenWrite(f2.FullName);
            stream2.Write(pBuf, 0, (int)pBuf.Length);
            stream2.Flush();
            stream2.Close();
        }
    }
