...
namespace 设备注册
{
    public partial class HIDDevice : Win32Usb, IDisposable
    {
        ...
        public static readonly ushort ProductID = 0x0729;
        public static readonly ushort ProductID_old = 0x000d;
        public static float SoftVersion = 20.05F;
        public static byte[] SwVer = new byte[2] { 0x20, 0x04 };
        ...
        public static HIDDevice FindDevice()
        {
            ...
            string strSearch1 = string.Format("vid_{0:x4}&pid_{1:x4}", VendorID, ProductID_old); // first, build the path search string
            Guid gHid;
            ...
            try
            {
                ...
                while (SetupDiEnumDeviceInterfaces(hInfoSet, 0, ref gHid, (uint)nIndex, ref oInterface))	// this gets the device interface information for a device at index 'nIndex' in the memory block
                {
                    ...
                    if (strDevicePath.IndexOf(strSearch) >= 0 || strDevicePath.IndexOf(strSearch1) >= 0)	// do a string search, if we find the VID/PID string then we found our device!
                    {
