...
{
	uint32_t x;
	curKeyStateFlags.Bits.bFindCar=bFindCarKey();
	curKeyStateFlags.Bits.bLock=bLockKey();
	curKeyStateFlags.Bits.bUnLock=bUnLockKey();
	curKeyStateFlags.Bits.bTailGate=bTailGateKey();
...
void RFIRTxProc(void)
{
	...
	//while(gFlags.Bits.bTxFinish==0);
	while(1)
	{
		if(gFlags.Bits.bTxFinish)
		{
			gTempIndex++;
			break;
		}
