...
void CDlgNEC_CHIP::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT1, m_strLog);
	DDX_Control(pDX, IDC_COMBO1, m_nRenewTimes);
}
...
BEGIN_MESSAGE_MAP(CDlgNEC_CHIP, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON2, &CDlgNEC_CHIP::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_BUTTON3, &CDlgNEC_CHIP::OnBnClickedButton3)
END_MESSAGE_MAP()
...
BOOL CDlgNEC_CHIP::OnInitDialog()
{
	...
	{
		str.Format(_T("%d"),i);
		m_nRenewTimes.AddString(str);
	}
	m_nRenewTimes.SetCurSel(6);
...
// 	ShowLog(strTemp);
	...
	for(int i=0;i<16;i++)
	{
		str+=CCommFunc::byteToHexStr(pBuf, i*16, 16, _T(" "));
		str+="\r\n";
	}
	m_strLog.SetWindowTextW(str);
...
void CDlgNEC_CHIP::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
	CString strTemp,strTemp1;
	CInteractionData Result;
	CCarKey3App::TheHIDDevice.KeyData_SetRenewTimes(m_nRenewTimes.GetCurSel()+1,&Result);
	if(Result.GetHIDResult()==_T(""))
	{
		strTemp.LoadString(IDS_RenewOK);
		ShowLog(strTemp);
...
void CDlgNEC_CHIP::OnBnClickedButton3()
{
	// TODO: 在此添加控件通知处理程序代码
	CString strTemp,strTemp1;
	CInteractionData Result;
	CCarKey3App::TheHIDDevice.KeyData_UnPRD(&Result);
	if(Result.GetHIDResult()==_T(""))
	{
		strTemp.LoadString(IDS_RenewOK);
		ShowLog(strTemp);
